...
class WEPay
{
    ...
    {
        $this->config = config("payment.wepay");
    }
    ...
    public function pay($order) {
        ...
        $data = [
            ...
            "notify_url" => url("api/payment/notify/pay/wepay", [], true),
            "page_url" => url("api/payment/notify/pay/wepay", [], true),
        ...
        if($result["respCode"] == "SUCCESS"){
            ...
            Log::info("WEPAY_PAY_ERROR:" . $result["tradeMsg"]);
            throw new \Exception("pay error");    
        }
    ...
    public function pay_notify_verify() {
        ...
        Log::debug("WEPAY_PAY_NOTIFY_REQUEST:", $returnArray);
        ...
        if ($sign == request("sign")) {
            ...
            if ($returncode == "1") {
                    ...
                    $msg = "WEPAY_NOTIFY_SUCCESS:订单号: " . $order_no;
                    Log::info($msg);
                    return $order_no;
            } else {
                Log::debug("WEPAY_PAY_NOTIFY_ERROR:RETURN CODE is " . $returncode);
                throw new \Exception("return code is " . $returncode);    
            }
        } else {
            Log::debug("WEPAY_PAY_NOTIFY_ERROR:SIGN ERROR");
    ...
    public function transfer($withdrawl) {
        ...
        $url = "https://pyabxum.weglobalex.com/pay/transfer";
        $data = [
            "mch_id" => $this->config["mid"],
            ...
            "back_url" => url("api/payment/notify/transfer/wepay", [], true),
        ];
        // 生成签名
        ...
        try {
            ...
            Log::info("WEPAY_TRANSFER_ERROR:" . $raw);
    ...
    public function transfer_notify_verify() {
        ...
        Log::debug("WEPAY_TRNASFER_NOTIFY_REQUEST:", $returnArray);
        ...
        if ($sign == request("sign")) {
            ...
            if ($returncode == "SUCCESS") {
                    ...
                    $msg = "WEPAY_TRNASFER_SUCCESS:订单号: " . $order_no;
                    Log::info($msg);
                    return $order_no;
            } else {
                Log::debug("WEPAY_TRNASFER_NOTIFY_ERROR:RETURN CODE is " . $returncode);
                throw new \Exception("return code is " . $returncode);    
            }
        } else {
            Log::debug("WEPAY_TRNASFER_NOTIFY_ERROR:SIGN ERROR");
            throw new \Exception("sign error");
